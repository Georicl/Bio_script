...
def setup_commands():
    ...
    def _(parser):
        ...
        """
        parser.add_argument("--fasta", required=True, help="参考基因组输入路径")
        parser.add_argument("--pair1", required=True, help="双端测序文件1输入路径")
        parser.add_argument("--pair2", required=True, help="双端测序文件2输入路径")
    ...
    def _(parser):
        ...
        parser.add_argument("--parallel", type=int, choices=[1, 2], default=2, help="是否启用多进程进行bedtools覆盖度计算，选择值为1、2。默认值为2（启用），1为不启用")
